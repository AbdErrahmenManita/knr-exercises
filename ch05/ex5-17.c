...
void field_start(unsigned char **s, int val) {
    int i = 0;
    while (**s != '\0' && i != val) {
        if (isspace(**s)) {
            i++;
...
int super_cmp(char *s, char *t, int *flags) {
    ...
    if (flags[4]) {
        field_start(&s1, flags[4] - 1);
        field_start(&t1, flags[4] - 1);
    }
    while (*s1 != '\0' && !isspace(*s1) || *t1 != '\0' && !isspace(*t1)) {
        if (flags[3]) {
...
int mastercmp(const char *s, const char *t, int *flags) {
    if (flags[0]) {
        return flags[1] * numcmp(s, t, flags[4] - 1);
...
int numcmp(const char *s, const char *t, int val) {
    field_start((unsigned char **)&s, val);
    field_start((unsigned char **)&t, val);
    double v1, v2;
    v1 = atof(s);
    v2 = atof(t);
    if (v1 < v2) {
...
void getflags(int nb_arg, char **arg, int *flags) {
    ...
    while (nb_arg > 0 && (*arg)[0] == '-') {
        while ((c = *++arg[0]) != '\0') {
            ...
            case 'k':
                if (has_field) {
                    fprintf(stderr, "Repeated the k argument\n");
                    exit(1);
                }
                if (isdigit(*(*arg + 1))) {
                    // check if the field N° is in the same argument
                    field_val = atoi(++(*arg));
                } else if (*(*arg + 1) == '\0' && nb_arg > 1 &&
                           isdigit(*(*(arg + 1)))) {
                    // check if the field N° is in the next argument
                    field_val = atoi(*++arg);
                    nb_arg--;
                } else {
                    fprintf(stderr, "Wrong Command : the k argument should be "
                                    "followed by the field depth\n");
                    exit(1);
                }
                while (isdigit(*(*arg + 1))) {
                    (*arg)++;
